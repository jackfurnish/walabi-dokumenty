...
export function InvestorSummarySection({ data }: InvestorSummarySectionProps) {
  ...
  return (
    <CollapsibleSection
      title={t('sections', 'investorSummary', lang)}
      subtitle={data.conceptTitle}
      badge={lang === 'pl' ? 'Gotowe na prezentację' : 'Presentation-ready'}
    ...
    >
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-4">
          ...
          <CopyButton text={summaryText} label={t('copy', 'copySummary', lang)} size="sm" />
        </div>

        {/* Elevator Pitch */}
        <div className="bg-stone-800 text-stone-100 rounded-xl px-6 py-5">
          <p className="label-overline text-stone-400 mb-3">
            {lang === 'pl' ? 'Elevator Pitch' : 'Elevator Pitch'}
          </p>
        ...
        <div>
          <p className="text-xs font-semibold text-stone-600 uppercase tracking-wide mb-2">
            {lang === 'pl' ? 'Narracja designu' : 'Design Narrative'}
          </p>
        ...
        <div className="bg-emerald-50/70 border border-emerald-100 rounded-xl p-5">
          <p className="text-xs font-semibold text-emerald-700 uppercase tracking-wide mb-2">
            {lang === 'pl' ? 'Wpływ na biznes' : 'Business Impact'}
          </p>
        ...
        <div>
          <p className="text-xs font-semibold text-stone-600 uppercase tracking-wide mb-3">
            {lang === 'pl' ? 'Proponowany zakres prac' : 'Proposed Scope of Work'}
          </p>
        ...
        <div className="grid grid-cols-2 gap-4 pt-3 border-t border-stone-100">
          <div>
            <p className="text-xs text-stone-400 mb-1">{lang === 'pl' ? 'Szacowany harmonogram' : 'Estimated Timeline'}</p>
            <p className="text-sm font-medium text-stone-800">{data.estimatedTimeline}</p>
          </div>
          <div>
            <p className="text-xs text-stone-400 mb-1">{lang === 'pl' ? 'Poprawa odczuć gości' : 'Guest Perception Upgrade'}</p>
        ...
        <div className="border-t border-stone-100 pt-4">
          <p className="text-xs text-stone-400 mb-1">{lang === 'pl' ? 'Rola WALABI' : 'WALABI Role'}</p>
